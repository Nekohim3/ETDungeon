...
namespace Assets._Scripts.AStar
{
        ...
        where T : IComparable<T>
    {
        ...
        {
            if (!map.TryGetValue(lookupFunc(value), out var index))
        ...
        private int LargestIndex(int i)
        {
            ...
            if (leftInd < collection.Count && collection[leftInd].CompareTo(collection[largest]) < 0)
                largest = leftInd;

            if (rightInd < collection.Count && collection[rightInd].CompareTo(collection[largest]) < 0)
                largest = rightInd;
